    ...
    if response_data == "google_search":
        ...
        if search_type == "weather":
            ...
            if "location" not in context.user_data:
                ...
                # Tạo URL tìm kiếm thời tiết (bạn có thể thay đổi query phù hợp)
            ...
            # Tìm kiếm sản phẩm: dùng query của người dùng để tạo link đến Google Shopping
            question = context.user_data.get("question", "")
            encoded_query = quote_plus(question)
            search_url = f"https://www.google.com/search?tbm=shop&q={encoded_query}"
            keyboard = InlineKeyboardMarkup([
                [InlineKeyboardButton("Mở kết quả mua sắm trên Google", url=search_url)]
            ])
            await query.edit_message_text("Đây là kết quả tìm kiếm sản phẩm:", reply_markup=keyboard)
            return ASK_RESPONSE_TYPE
        else:
