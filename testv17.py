...
async def ask_response_type(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Lấy câu hỏi: ưu tiên update.message.text, nếu không có thì dùng context.user_data["question"]
    question = (
        update.message.text 
        if update.message and update.message.text 
        else context.user_data.get("question", "")
    )
    
    if not question:
        await update.message.reply_text("Không có câu hỏi. Vui lòng nhập câu hỏi.")
        return ASK_RESPONSE_TYPE
...
async def voice_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Xử lý tin nhắn giọng nói từ người dùng:
    - Tải file giọng nói từ Telegram.
    - Chuyển đổi file từ định dạng OGG sang WAV (dùng pydub).
    - Dùng SpeechRecognition chuyển giọng nói thành văn bản.
    - Gán văn bản nhận diện được cho câu hỏi và tiếp tục flow Q&A.
    """
    translation = load_language(context.user_data.get("language", "en"))
    ...
    temp_ogg = os.path.join(SAVE_DIR, f"voice_{uuid.uuid4()}.ogg")
    await file.download_to_drive(custom_path=temp_ogg)

    # Chuyển đổi file OGG sang WAV (pydub yêu cầu ffmpeg được cài đặt)
    try:
        audio = AudioSegment.from_file(temp_ogg, format="ogg")
        temp_wav = temp_ogg.replace(".ogg", ".wav")
        audio.export(temp_wav, format="wav")
    except Exception as e:
        await update.message.reply_text(f"{translation.get('audio_error', 'Lỗi chuyển đổi âm thanh')}: {str(e)}")
        return ASK_RESPONSE_TYPE

    # Sử dụng SpeechRecognition để chuyển giọng nói thành văn bản
    recognizer = sr.Recognizer()
    with sr.AudioFile(temp_wav) as source:
        audio_data = recognizer.record(source)
    try:
        # Lấy ngôn ngữ từ context (nếu đã thiết lập, ví dụ 'vi' hay 'en')
        lang_code = context.user_data.get("language", "en")
        # Chuyển đổi: ví dụ, nếu 'vi' thì chuyển thành "vi-VN"
        google_lang = "vi-VN" if lang_code == "vi" else "en-US"
        recognized_text = recognizer.recognize_google(audio_data, language=google_lang)
    except sr.UnknownValueError:
        await update.message.reply_text(translation.get("voice_not_understood", "Không nhận diện được giọng nói. Vui lòng thử lại."))
        return ASK_RESPONSE_TYPE
    except sr.RequestError as e:
        await update.message.reply_text(f"{translation.get('audio_error', 'Lỗi dịch vụ nhận diện')}: {e}")
        return ASK_RESPONSE_TYPE
    ...
    await update.message.reply_text(f"{translation.get('recognized_text', 'Văn bản nhận diện được')}: {recognized_text}")

    # Gán văn bản nhận diện được vào câu hỏi và gọi hàm xử lý câu hỏi hiện có
    context.user_data["question"] = recognized_text
    return await ask_response_type(update, context)
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            ASK_RESPONSE_TYPE: [
                ...
                MessageHandler(filters.VOICE, voice_handler)  # Thêm handler cho giọng nói
            ]
        },
